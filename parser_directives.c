...
/*   Updated: 2020/01/15 19:04:50 by ashishae         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	parse_ambient(char *line, t_scene *scene, char **splitted)
{
	...
	if (!check_color2(scene->ambient_color))
		handle_error("Ambient color components must be in range [0-180].");
}
...
void	parse_camera(char *line, t_scene *scene, char **splitted)
{
	...
	if (!check_normal(camera->direction))
		handle_error("Camera orientation must be normalized.");
